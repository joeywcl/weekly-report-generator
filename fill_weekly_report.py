...
def prompt_ai_task(task_num):
    ...
    insight_failure = prompt("Insight / Failure", required=True)
    return {
        "task": task,
...
def main():
    ...
    print("-"*60)
    weekly_objective = prompt("Weekly Objective (One Sentence)", prev_data.get("weekly_objective", ""))
    ...
    print("-"*60)
    sop_item = prompt("Item", prev_data.get("sop_process_solidification", {}).get("item", "None"))
    sop_impact = prompt("Impact", prev_data.get("sop_process_solidification", {}).get("impact", "N/A"))
    ...
    data = {
        ...
        "transformation_log": {
            "ai_acceleration_tasks": ai_tasks
        },
        "sop_process_solidification": {
            "item": sop_item,
            "impact": sop_impact
        },
